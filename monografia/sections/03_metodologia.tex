...
A escolha dessa plataforma justifica-se por: (i) acesso completo aos dados de \textit{analytics} e métricas de negócio; (ii) possibilidade de implementação controlada de modificações técnicas; (iii) volume de tráfego suficiente para análise estatística significativa; (iv) representatividade do segmento de \textit{e-learning} no mercado americano; (v) diversidade de modelos de monetização que permite análise comparativa; e (vi) histórico de migração arquitetural que possibilita análise de impacto em performance.
